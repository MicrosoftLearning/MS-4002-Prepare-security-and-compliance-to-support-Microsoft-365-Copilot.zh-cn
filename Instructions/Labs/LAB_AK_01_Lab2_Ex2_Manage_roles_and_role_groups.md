...
1. 在前面的实验室练习中，你在 LON-DC1 上为 Adatum 创建了一个新域。 现在，你应切回 LON-CL1 以执行本实验室练习中的 Microsoft 365 管理任务。**** 作为最佳做法，典型的 Microsoft 365 管理任务应在客户端电脑上而不是公司的域控制器上执行。  <br/>
...
在此任务中，Holly 希望将 Patti Fernandez 分配到服务支持管理员角色。 若要使用 Microsoft Graph PowerShell 模块将用户添加到管理员角色，必须先获取该用户的对象 ID 和角色的对象 ID。 在此任务中，你将使用 Microsoft Graph 来获取相应的 ID，然后创建角色分配。 
...
2. 在前面的实验室练习中，你已连接到 Microsoft Graph 并请求了“Directory.ReadWrite.All”权限来运行还原了已删除的组的 cmdlet。 若要更新此任务中的用户和角色对象，Holly 现在必须为每个对象请求读取/写入权限。 若要请求这些权限，请在命令提示符处键入以下命令，然后按 Enter：  <br/>

        Connect-MgGraph -Scopes 'User.ReadWrite.All', 'RoleManagement.ReadWrite.Directory'

3. 在显示的“选取帐户”窗口中，选择 Holly Dickson 的帐户。****

4. 在显示的“请求权限”对话框中，选中“代表组织同意”复选框，然后选择“接受”。************

5. Holly 希望将 Patti Fernandez 分配到“服务支持管理员”角色。******** 若要使用 Microsoft Graph PowerShell 分配此角色，必须先获取服务支持管理员角色的对象 ID，以便你可以将其分配给 Patti。

    **重要提示：** 在执行此步骤之前，请验证 PowerShell 窗口是否处于全屏模式。 角色名会显示在输出的第二列中。 如果未处于全屏模式，则你看不到与每个对象 ID 关联的角色名称。 

6. 若要在 Microsoft Graph PowerShell 中分配角色，必须先找到其对象 ID。 可通过两种方式查看角色模板 - 可以显示整个角色模板列表，也可以显示特定角色的模板。 作为学习体验，你将执行这两种方法，以便了解差异。 <br/>
    ...
    运行此命令后可以看到，你必须滚动浏览角色模板列表来查找服务支持管理员角色。 不难看出，即使对表格进行了排序，这样做也很繁琐。 或者，运行以下命令以查询特定角色模板 - 在本例中即“服务支持管理员”角色模板： <br/>
...
7. 若要将 Patti Fernandez 分配到新启用的服务支持管理员角色，必须先获取 Patti 用户帐户的对象 ID。 为此，请键入以下命令并按 Enter：

        Get-MgUser -Filter "DisplayName eq 'Patti Fernandez'" | Format-Table ID, DisplayName

8. 现在你知道了服务支持管理员角色的对象 ID 和 Patti 用户帐户的对象 ID，可以将该角色分配给 Patti。 执行以下步骤来完成此过程，将两个 ID 替换为前面两个步骤中显示的值。
    ...
    **备注：** DirectoryScopeId 参数表示分配是在租户/目录级别。

9. 你现在想要验证 Patti 是否已被分配到服务支持管理员角色。 为此，我们将运行两个命令。 第一个获取分配的角色并将其存储在变量中，第二个显示分配给该角色的用户 ID。
...
10. 前面的命令只显示了分配给所选角色的用户的 ID。 但是，你可以将显示的 ID 与 Patti 的 ID 匹配，以验证是否已为其帐户分配了“服务支持管理员”角色。**** 可以看到，Patti 是分配给该角色的唯一用户。 

11. 现在，让我们重复此过程，查看分配给全局管理员角色的所有用户。

        $assignedRole = Get-MgDirectoryRole -Filter "DisplayName eq 'Global Administrator'"
        Get-MgDirectoryRoleMember -DirectoryRoleId $assignedRole.Id

12. 验证是否有多个已分配到全局管理员角色的 Adatum 用户。 在实际方案中，Microsoft 365 管理员会使用此 PowerShell 命令监视其 Microsoft 365 部署中存在的全局管理员数量。 然后，他们将为任何真正不应该拥有全局管理员的用户移除此角色（请记住，最佳做法准则是在 Microsoft 365 部署中拥有 2 到 4 名全局管理员，具体取决于组织的大小）。  <br/>
...
13. 让 Windows PowerShell 会话保持打开状态以供将来的实验室练习使用，但在执行下一个任务之前，请将其最小化。
...
8. 在 **LON-CL2** 中的登录屏幕上，你将以本地 **LON-CL2\Admin** 帐户身份登录，密码为 **Pa55w.rd**。
